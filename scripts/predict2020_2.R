...
# includes through Oct 28
covid_state_df <- read_csv("United_States_COVID-19_Cases_and_Deaths_by_State_over_Time.csv")
poll_2020_df <- read_csv("presidential_poll_averages_2020.csv") # updated Oct 28
...
namevector4 <- c("demog_model_coef")
namevector5 <- c("lwr")
namevector6 <- c("upr")
...
state_predictions[ , c(namevector1, namevector2, namevector3, namevector4, namevector5, 
                       namevector6, namevector7, namevector8, namevector9, namevector10)] <- NA

#lm(R_pv2p ~ Female_change + Black_change, data = demog_dat_change)
#lm(pct_trend_adjusted ~ tot_death_adj, data = covid_dat)
...
predict.function <- function(state_name) {
  ...
  state_predictions$demog_model_coef[state_predictions$states_list == state] <<- state_demog_model$coef[2]
  old_coef_Femlae <- state_demog_model$coef[2]
  ...
  state_prediction <- 0.6*predict(state_poll_model, state_2020_true) + 
                      ...
                      #+ (new_coef_Female-old_coef_Female)new_value_Feamle_change)
...
hist3 <- ggplot(state_predictions, aes(x = demog_model_coef)) + 
...
coef_hist <- ggarrange(hist1, hist2, hist3, ncol = 3, nrow = 1)

annotate_figure(coef_hist,
                top = text_grob("Model Coefficients", face = "bold", size = 20))
ggsave("final_pred_coef.png", width = 16, height = 4)
